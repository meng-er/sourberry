<template>
  <div id="counselList">
    ...
    >
      ...
      <el-table-column prop="conName" label="姓名">
        <template slot-scope="scope">
      ...
      <el-table-column prop="deName" label="日期">
        <template slot-scope="scope">
          <span>{{ scope.row.seektime.split(" ")[0] }}</span>
        </template>
      </el-table-column>
      <el-table-column prop="deName" label="问题描述">
        <template slot-scope="scope">
          <span>{{ scope.row.description }}</span>
        </template>
      </el-table-column>
      <el-table-column prop="deName" label="主要症状">
        <template slot-scope="scope">
          <span>{{ scope.row.diagnosis }}</span>
        </template>
      </el-table-column>

      <el-table-column prop="deName" label="所用药物">
        <template slot-scope="scope">
    ...
    <div class="beatch_remove">
      <el-button @click="toggleSelection()" round type="danger"
        >批量删除</el-button
      >
    </div>
...
export default {
  ...
  computed: {
    tables() {
      return this.counselList.filter((item) => {
        return (
          item.userinfo.username.includes(this.formInline.userName) &&
          item.recordCode.includes(this.formInline.num) &&
